...
const dragReceiverMarker = L.icon({
  ...
  popupAnchor: [2, -40],
  iconUrl:
    "https://cdn-icons-png.flaticon.com/512/1180/1180058.png?w=740&t=st=1675612962~exp=1675613562~hmac=72aeaef81f5c310472a9da30bdcdfee7807d60f8593016c05f9460f1037eae64",
});
const Map = () => {
  const { senderLocationLatLng, receiverLocationLatLng } = useSelector(
  ...
  const calculateDistance = () => {
    ...
    const distance = R * c;
    const pricePerKm = 100; // Price per kilometer in Rs
  const totalCost = distance * pricePerKm;
  notification.open({
    message: `Distance between Sender and Receiver is: ${distance.toFixed(2)} km`,
    description: `Total cost: Rs ${totalCost.toFixed(2)}`,
  });
  ...
  const geoCodeLatLng = (lat, lng) => {
    fetch(
      `https://api.geoapify.com/v1/geocode/reverse?lat=${lat}&lon=${lng}&apiKey=${process.env.REACT_APP_MAP_API_KEY}`
    )
      .then((res) => res.json())
      .then((data) =>
        notification.open({
          message: `${data.features[0].properties.formatted}`,
        })
      );
  };
  ...
  function SenderDraggableMarker() {
    ...
    return (
      ...
      >
        <Popup minWidth={90}>
          <span>This marker is draggable</span>
        </Popup>
      </Marker>
    );
  }
  ...
  return (
    <>
      <MapContainer
        center={lat ? [lat, lng] : [27.68564550564005, 85.3445145828365]}
        zoom={10}
      ...
      >
        ...
        {lat && (
          <Polyline
            color="#003312"
            positions={[senderLocationLatLng, receiverLocationLatLng]}
          />
        )}
      </MapContainer>
    </>
